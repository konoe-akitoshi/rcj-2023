...
obj_color_orange = [(14, 100, 25, 127, 0, 127)]  # Orange Ball
obj_color_yellow = [(20, 100, -10, 0, 16, 65)]  # Yellow Goal
obj_color_blue = [(0, 30, -20, 127, -128, -10)]  # Blue Goal
...
#   "area_threshold" are returned by "find_blobs" below.
# - Change "pixels_threshold" and "area_threshold" if you change the camera
#   resolution.
# - Don't set "merge = True" becuase that will merge blobs which we don't want
#   here.
...
while True:
    ...
    # check Orange Ball
    i_o = 0
    balls_o1 = img.find_blobs(
        obj_color_orange, pixels_threshold=10, area_threshold=10)
    balls_o2 = sorted(balls_o1, key=lambda x: x.area())
    if balls_o1 is not None:
    ...
    # check Yellow Goal
    i_y = 0
    balls_y1 = img.find_blobs(
        obj_color_yellow, pixels_threshold=250, area_threshold=250
    )  # 元値500
    balls_y2 = sorted(balls_y1, key=lambda x: x.area())
    if balls_y1 is not None:
    ...
    # check Blue Goal
    i_b = 0
    balls_b1 = img.find_blobs(
        obj_color_blue, pixels_threshold=250, area_threshold=250)
    balls_b2 = sorted(balls_b1, key=lambda x: x.area())
    if balls_b1 is not None:
